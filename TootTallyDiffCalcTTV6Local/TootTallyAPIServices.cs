...
namespace TootTallyDiffCalcTTV6Local
{
    public static class TootTallyAPIServices
    {
        ...
        {
            var response = GetStringRequestOld($"songs/rated");
            //if (response.IsFaulted || response.IsCanceled || !response.IsCompletedSuccessfully) return new List<int>();
            return JsonConvert.DeserializeObject<List<int>>(response.Result);
        }
        ...
        public static void GetTmbJsonFromURL(string url, Action<string, string> callback)
        {
            try
            {
                callback(GetStringDownloadRequest(url).Result, url);
            }
            catch (Exception)
            {
                Trace.WriteLine("Couldn't find tmb for " + url);
            }
        }
